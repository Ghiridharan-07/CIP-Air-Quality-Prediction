...
def load_data(filepath):
    ...
    df['City'] = df['City'].str.strip()  # Remove trailing spaces
    df = df[df['City'] == 'Delhi'].copy()

    if df.empty:
        raise ValueError("❌ Dataframe is empty after filtering for 'Delhi'. Check city name or data.")
...
# -----------------------------
def main():
    path = r"C:\Users\S.GHIRIDHARAN\Downloads\city_day.csv"  # Update if needed
    df = load_data(path)
